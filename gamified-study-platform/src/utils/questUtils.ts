...
export const questUtils = {
  ...
  generateQuestDescription(
    ...
    switch (type) {
      case 'daily':
        description = 'Complete this daily quest to maintain your study streak and earn XP.';
        break;
      case 'weekly':
        description = 'A week-long challenge to deepen your understanding and build consistent study habits.';
        break;
      case 'milestone':
        description = 'A significant achievement that demonstrates mastery of important concepts.';
        break;
      case 'bonus':
        description = 'Special bonus quest with extra XP rewards for going above and beyond.';
        break;
    }
    ...
    switch (difficulty) {
      case 'easy':
        description += ' This is an easy quest that should be quick to complete.';
        break;
      case 'medium':
        description += ' This quest requires moderate effort and focus.';
        break;
      case 'hard':
        description += ' This challenging quest will test your dedication and understanding.';
        break;
    }
  ...
  generateRequirements(
    ...
    switch (type) {
      case 'daily':
        ...
        requirements.push({
          type: 'study_time',
          target: difficulty === 'easy' ? 15 : difficulty === 'medium' ? 30 : 45,
          current: 0,
      ...
      case 'weekly':
        ...
        requirements.push({
          type: 'study_time',
          target: difficulty === 'easy' ? 60 : difficulty === 'medium' ? 120 : 180,
          current: 0,
      ...
      case 'milestone':
        ...
        requirements.push({
          type: 'study_time',
          target: difficulty === 'easy' ? 90 : difficulty === 'medium' ? 180 : 300,
          current: 0,
  ...
  generateQuest(
    ...
    const description = this.generateQuestDescription(type, difficulty, requirements, topic);
   ...
   */
  generateQuestsForCourse(courseId: string, syllabus: SyllabusItem[]): Partial<Quest>[] {
    ...
    activeItems.slice(0, 2).forEach(item => {
      ...
      quests.push(
        this.generateQuest('daily', 'easy', courseId, item.title)
      );
    ...
    activeItems.slice(0, 3).forEach(item => {
      const difficulty: QuestDifficulty = 
        item.priority === 'high' ? 'hard' : 
        item.priority === 'medium' ? 'medium' : 'easy';
    ...
    const highPriorityItems = activeItems.filter(item => item.priority === 'high');
    if (highPriorityItems.length > 0) {
      quests.push(
        this.generateQuest('bonus', 'hard', courseId, highPriorityItems[0].title)
      );
    }
   ...
   */
  calculateCourseWeights(courses: Course[]): { courseId: string; weight: number }[] {
    ...
    for (const course of courses) {
      ...
      if (itemsWithDeadlines.length > 0) {
        ...
        const closestDeadline = new Date(Math.min(
          ...itemsWithDeadlines.map(item => item.deadline!.getTime())
        ));
        
        const daysUntilDeadline = Math.max(1, 
          Math.ceil((closestDeadline.getTime() - Date.now()) / (1000 * 60 * 60 * 24))
        );
  ...
  generateBalancedQuests(courses: Course[]): Partial<Quest>[] {
    ...
    for (const { courseId, count } of questsPerCourse) {
      ...
      for (const type of questTypes) {
        ...
        // Generate the quest
        quests.push(
          this.generateQuest(type, difficulty, courseId, item.title)
        );
      }
    }
  ...
  calculateQuestProgress(quest: Quest): number {
    ...
    return Math.min(100, Math.round((totalProgress / quest.requirements.length) * 100));
  }
};
