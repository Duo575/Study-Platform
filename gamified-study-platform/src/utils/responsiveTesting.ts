...
export class ResponsiveDesignTester {
  ...
  private checkResponsiveIssues(
      ...
      interactiveElements.forEach(element => {
        ...
        if (rect.width < minSize || rect.height < minSize) {
          issues.push({
            ...
            suggestion:
              'Increase padding or size for better touch accessibility',
          });
        }
      });
    }
    ...
    textElements.forEach(element => {
      ...
      const minSize =
        minFontSizes[breakpoint as keyof typeof minFontSizes] || 16;
    ...
    const elementRects = Array.from(allElements)
      .map(el => ({
        element: el,
    ...
    contentElements.forEach(element => {
      ...
      if (rect.width > width * 0.95) {
        // More than 95% of viewport width
        issues.push({
          type: 'warning',
          rule: 'content-width',
  ...
  private generateRecommendations(
    ...
    if (issueTypes.has('horizontal-scroll')) {
      recommendations.push(
        `Eliminate horizontal scrolling on ${breakpoint} by using flexible layouts and proper overflow handling.`
      );
    }

    if (issueTypes.has('touch-target-size')) {
      recommendations.push(
        `Increase touch target sizes to at least 44px for better ${breakpoint} usability.`
      );
    }

    if (issueTypes.has('font-size')) {
      recommendations.push(
        `Use appropriate font sizes for ${breakpoint} devices to ensure readability.`
      );
    }

    if (issueTypes.has('element-overlap')) {
      recommendations.push(
        `Review layout spacing to prevent element overlap on ${breakpoint}.`
      );
    }

    if (issueTypes.has('content-width')) {
      recommendations.push(
        `Ensure content scales appropriately for ${breakpoint} viewport width.`
      );
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        `Great responsive design for ${breakpoint}! No issues found.`
      );
    }
    ...
    tables.forEach(table => {
      ...
      if (rect.width > viewportWidth * 0.9) {
        issues.push({
          ...
          element: table,
          suggestion:
            'Consider responsive table patterns like horizontal scrolling or card layout',
        });
      }

      // Check if table has responsive wrapper
      const wrapper = table.closest(
        '.table-responsive, .overflow-x-auto, [style*="overflow"]'
      );
