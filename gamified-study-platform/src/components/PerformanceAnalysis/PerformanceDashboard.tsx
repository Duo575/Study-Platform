...
export function PerformanceDashboard({
  ...
  const _criticalSubjects = getSubjectsByStatus('critical');
  const _excellentSubjects = getSubjectsByStatus('excellent');
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="text-2xl font-bold text-yellow-600">
              {summary.subjectsNeedingAttention}
            </div>
      ...
      {flaggedSubjects.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
          <h3 className="text-red-800 font-medium mb-2">
            ⚠️ Subjects Requiring Immediate Attention
          </h3>
              ...
              <div key={subject.courseId} className="text-red-700 text-sm">
                ...
                {subject.lastStudied && (
                  <span className="ml-2 text-red-600">
                    (Last studied:{' '}
                    {Math.floor(
                      (Date.now() - subject.lastStudied.getTime()) /
                        (24 * 60 * 60 * 1000)
                    )}{' '}
                    days ago)
                  </span>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        {performances.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            <p>
              No subjects found. Add some courses to see performance analysis.
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-sm text-gray-600 mt-1">
                      {priority.timeAllocation}% of study time
                    </div>
                  </div>
                </div>
              ))}
  ...
  return (
    <div className="p-4">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <div className="flex items-center space-x-4 mt-1">
            ...
            <span
              className={`inline-block px-2 py-1 rounded text-xs font-medium ${getPerformanceStatusColor(performance.status)}`}
            >
              {performance.status.replace('_', ' ').toUpperCase()}
            </span>
            <span
              className={`text-lg font-bold ${getGradeColor(performance.overallGrade)}`}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
        <div className="text-center">
          <div className="text-sm font-medium text-gray-900">
            {performance.studyTimeScore}/100
          </div>
          <div className="text-xs text-gray-600">Study Time</div>
        </div>
        <div className="text-center">
          <div className="text-sm font-medium text-gray-900">
            {performance.questCompletionScore}/100
          </div>
          <div className="text-xs text-gray-600">Quest Completion</div>
        </div>
        <div className="text-center">
          <div className="text-sm font-medium text-gray-900">
            {performance.consistencyScore}/100
          </div>
          <div className="text-xs text-gray-600">Consistency</div>
        </div>
        <div className="text-center">
          <div className="text-sm font-medium text-gray-900">
            {performance.deadlineAdherenceScore}/100
          </div>
      ...
      <div className="flex items-center space-x-6 text-sm text-gray-600">
        ...
        <span>
          ✅ {performance.completedQuests}/{performance.totalQuests} quests
        </span>
        <span>
          📖 {performance.completedTopics}/{performance.totalTopics} topics
        </span>
        <span>📊 {performance.studyFrequency.toFixed(1)}x/week</span>
      </div>

      {/* High Priority Recommendations */}
      {performance.recommendations.filter(r => r.priority === 'high').length >
        0 && (
