...
const QuestProgress: React.FC<QuestProgressProps> = ({ className = '' }) => {
  ...
  const potentialXp = activeQuests.reduce((sum, quest) => sum + quest.xpReward, 0);
  ...
  const questsByType = quests.reduce((acc, quest) => {
    ...
    return acc;
  }, {} as Record<QuestType, { total: number; completed: number; active: number }>);
  
  // Calculate average progress for active quests
  const averageProgress = activeQuests.length > 0 
    ? activeQuests.reduce((sum, quest) => sum + questUtils.calculateQuestProgress(quest), 0) / activeQuests.length
    : 0;
  ...
  const questsExpiringSoon = activeQuests.filter(quest => {
    ...
    const hoursUntilExpiry = (quest.expiresAt.getTime() - Date.now()) / (1000 * 60 * 60);
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-600">{totalXpEarned}</div>
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="text-xl font-bold text-yellow-600">{totalXpEarned} XP</div>
          </div>
          <div>
            <div className="text-sm text-gray-600 mb-1">Potential XP (Active Quests)</div>
            <div className="text-xl font-bold text-orange-600">{potentialXp} XP</div>
          </div>
        </div>
        ...
        {potentialXp > 0 && (
          <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
            <p className="text-sm text-yellow-800">
              Complete all active quests to earn <strong>{potentialXp} XP</strong> more!
            </p>
          </div>
        )}
      </Card>
        ...
        <Card className="p-4 border-orange-200 bg-orange-50">
          ...
          <div className="space-y-2">
            {questsExpiringSoon.map((quest) => {
              const hoursLeft = Math.ceil((quest.expiresAt!.getTime() - Date.now()) / (1000 * 60 * 60));
              return (
                <div key={quest.id} className="flex items-center justify-between p-2 bg-white rounded border">
                  <div className="flex items-center gap-2">
          ...
          <div className="text-center">
            {completionRate >= 80 ? (
              <div>
                ...
                <p className="font-semibold text-blue-800">Excellent progress!</p>
                <p className="text-sm text-blue-600">You're crushing your quest goals!</p>
              </div>
            ) : completionRate >= 50 ? (
              <div>
                <div className="text-2xl mb-2">💪</div>
                <p className="font-semibold text-purple-800">Great momentum!</p>
                <p className="text-sm text-purple-600">Keep up the fantastic work!</p>
              </div>
            ) : completionRate >= 25 ? (
              <div>
                <div className="text-2xl mb-2">🚀</div>
                <p className="font-semibold text-indigo-800">You're getting started!</p>
                <p className="text-sm text-indigo-600">Every quest completed is progress!</p>
              </div>
            ) : (
              <div>
                <div className="text-2xl mb-2">🌟</div>
                <p className="font-semibold text-gray-800">Ready to begin?</p>
                <p className="text-sm text-gray-600">Start with your first quest today!</p>
              </div>
            )}
          </div>
        </Card>
