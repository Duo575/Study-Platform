...
const QuestNotification: React.FC<QuestNotificationProps> = ({
  ...
  const [previousActiveQuestIds, setPreviousActiveQuestIds] = useState<
    string[]
  >([]);
  const [previousCompletedQuestIds, setPreviousCompletedQuestIds] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (previousActiveQuestIds.length > 0) {
      const newQuestIds = currentActiveQuestIds.filter(
        id => !previousActiveQuestIds.includes(id)
      );

      if (newQuestIds.length > 0) {
        const latestNewQuest =
          activeQuests.find(q => q.id === newQuestIds[0]) || null;
        if (latestNewQuest) {
    ...
    if (previousCompletedQuestIds.length > 0) {
      ...
      if (newCompletedQuestIds.length > 0) {
        const latestCompletedQuest =
          completedQuests.find(q => q.id === newCompletedQuestIds[0]) || null;
  ...
  const handleViewNewQuest = () => {
    if (newQuest && _onViewQuest) {
      _onViewQuest(newQuest);
    }
    setShowNewQuestToast(false);
  };

  const handleViewCompletedQuest = () => {
    if (completedQuest && _onViewQuest) {
      _onViewQuest(completedQuest);
    }
    setShowCompletedToast(false);
  };
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Badge className={getQuestTypeColor(completedQuest.type)}>
                {completedQuest.type.charAt(0).toUpperCase() +
                  completedQuest.type.slice(1)}
              </Badge>
              <span className="text-yellow-500 font-bold">
                +{completedQuest.xpReward} XP
              </span>
            </div>
            <p className="font-medium">{completedQuest.title}</p>
            <p className="text-sm text-gray-600">
              You've successfully completed this quest!
            </p>
            <div className="flex justify-end">
