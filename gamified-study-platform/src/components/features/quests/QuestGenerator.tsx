...
const QuestGenerator: React.FC<QuestGeneratorProps> = ({ onClose }) => {
  ...
  const [generationType, setGenerationType] = useState<'single' | 'balanced'>('balanced');
  ...
  const handleGenerate = async () => {
    ...
    try {
      if (generationType === 'single' && selectedCourseId) {
        ...
        const quests = await generateQuestsForCourse(selectedCourseId, selectedCourse.syllabus);
    ...
    <Card className="p-6">
      ...
      <div className="space-y-4 mb-6">
        ...
        {generationType === 'balanced' && (
          <div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-800">
            <p>
              <strong>Balanced Generation:</strong> This will create quests across all your courses, 
              prioritizing courses with upcoming deadlines, high priority topics, and lower completion rates.
            </p>
          </div>
        )}
        ...
        {generationType === 'single' && selectedCourse && (
          <div className="bg-gray-50 border border-gray-200 rounded p-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                <span className="text-gray-600">Topics:</span> {selectedCourse.syllabus.length}
              </div>
              <div>
                <span className="text-gray-600">Completion:</span> {selectedCourse.progress.completionPercentage}%
              </div>
              <div>
                <span className="text-gray-600">Hours Studied:</span> {selectedCourse.progress.hoursStudied}
              </div>
              <div>
                <span className="text-gray-600">Topics Completed:</span> {selectedCourse.progress.topicsCompleted}/{selectedCourse.progress.totalTopics}
              </div>
            </div>
            
            <div className="mt-2">
              <h4 className="text-sm font-medium mb-1">Quest Types You'll Get:</h4>
      ...
      <div className="flex justify-end space-x-3">
        ...
        <Button 
          ...
          disabled={isGenerating || (generationType === 'single' && !selectedCourseId)}
