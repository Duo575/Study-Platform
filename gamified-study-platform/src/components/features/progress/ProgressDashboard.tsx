...
export function ProgressDashboard({
  ...
  const progressStats = useMemo(() => {
    ...
    const completedCourses = courses.filter(c => c.progress.completionPercentage === 100).length
    const inProgressCourses = courses.filter(c => c.progress.completionPercentage > 0 && c.progress.completionPercentage < 100).length
    ...
    return {
      ...
      averageCourseProgress: totalCourses > 0 
        ? courses.reduce((sum, c) => sum + c.progress.completionPercentage, 0) / totalCourses 
        : 0
    }
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                Avg: {formatDuration(analytics.averageSessionLength)} per session
              </p>
            </div>
        ...
        <Card variant="elevated">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-2">
                <ProgressBar 
                  progress={(analytics.levelProgress.xpProgress / analytics.levelProgress.xpToNextLevel) * 100}
                  color="purple"
                  height="h-2"
                />
                <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                  {analytics.levelProgress.xpProgress} / {analytics.levelProgress.xpToNextLevel} XP
                </p>
              </div>
            </div>
        ...
        <Card variant="elevated">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-2">
                <ProgressBar 
                  progress={(progressStats.unlockedAchievements / progressStats.totalAchievements) * 100}
                  color="green"
                  height="h-2"
                />
                <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                  {Math.round((progressStats.unlockedAchievements / progressStats.totalAchievements) * 100)}% unlocked
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <WeeklyGoals 
                currentWeekStats={{
                  studyTime: analytics.weeklyProgress[analytics.weeklyProgress.length - 1]?.studyTime || 0,
                  questsCompleted: analytics.weeklyProgress[analytics.weeklyProgress.length - 1]?.questsCompleted || 0,
                  xpEarned: analytics.weeklyProgress[analytics.weeklyProgress.length - 1]?.xpEarned || 0
                }}
              />
            </CardContent>
          </Card>
                ...
                {courses.map((course) => (
                  <div key={course.id} className="border-b border-gray-200 dark:border-gray-700 pb-4 last:border-b-0">
                    ...
                    <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
                      <span>{course.progress.topicsCompleted}/{course.progress.totalTopics} topics</span>
                      <span>{formatDuration(course.progress.hoursStudied)} studied</span>
                    </div>
                  </div>
                ))}
              </div>
