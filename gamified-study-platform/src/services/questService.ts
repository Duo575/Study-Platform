...
export const questService = {
   ...
   */
  generateDailyQuests(courseId: string, syllabus: SyllabusItem[]): Partial<Quest>[] {
    ...
    if (activeTopics[0].topics.length > 0) {
      quests.push(
        questUtils.generateQuest('daily', 'medium', courseId, activeTopics[0].topics[0])
      );
    }
   ...
   */
  generateWeeklyQuests(courseId: string, syllabus: SyllabusItem[]): Partial<Quest>[] {
    // Get active topics (not completed)
    const activeTopics = syllabus.filter(item => !item.completed);
    ...
    quests.push(
      questUtils.generateQuest('weekly', 'medium', courseId, activeTopics[0].title)
    );
    
    // Generate a hard weekly consistency quest
    quests.push({
      title: 'Weekly Consistency Challenge',
      description: 'Maintain your study streak for 5 days this week to build strong study habits',
      type: 'weekly',
      difficulty: 'hard',
   ...
   */
  generateMilestoneQuests(courseId: string, syllabus: SyllabusItem[]): Partial<Quest>[] {
    const quests: Partial<Quest>[] = [];
    ...
    for (const item of syllabus) {
      ...
      const quest = questUtils.generateQuest('milestone', difficulty, courseId, item.title);
      ...
      const milestoneQuest: Partial<Quest> = {
        ...
        expiresAt: item.deadline || questUtils.generateExpirationDate('milestone')
      };
  ...
  generateBonusQuests(courseId: string): Partial<Quest>[] {
    ...
    quests.push({
      title: 'Streak Master',
      description: 'Maintain a 10-day study streak for a massive XP bonus and improved retention',
      type: 'bonus',
      difficulty: 'hard',
    ...
    quests.push({
      title: 'Task Champion',
      description: 'Complete 20 todo items for a special reward and productivity boost',
      type: 'bonus',
      difficulty: 'medium',
  ...
  async updateQuestProgress(
    ...
    try {
      ...
      const allRequirementsMet = updatedRequirements.every(req => req.current >= req.target);
      ...
      return {
        ...
        expiresAt: updatedQuest.expires_at ? new Date(updatedQuest.expires_at) : undefined,
        completedAt: updatedQuest.completed_at ? new Date(updatedQuest.completed_at) : undefined
      };
    } catch (error) {
