...
import { TodoItem, TodoForm, TodoFilters, APIResponse, PaginatedResponse } from '../types'
...
export class TodoService {
   ...
   * Update a todo item
   */
  static async updateTodo(id: string, updates: Partial<TodoForm>): Promise<APIResponse<TodoItem>> {
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) throw new Error('User not authenticated')
   ...
   */
  static async toggleTodo(id: string): Promise<APIResponse<{ todo: TodoItem; xpEarned: number }>> {
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) throw new Error('User not authenticated')
      ...
      if (newCompletedStatus) {
        xpEarned = this.calculateXPReward(currentTodo.priority, currentTodo.estimated_minutes)
      ...
      return {
        ...
        message: newCompletedStatus ? 'Todo completed! XP awarded.' : 'Todo marked as incomplete',
        success: true,
        timestamp: new Date().toISOString(),
      }
    } catch (error) {
      console.error('Error toggling todo:', error)
      throw new Error(error instanceof Error ? error.message : 'Failed to toggle todo')
    }
  }
   ...
   * Get todo statistics
   */
  static async getTodoStats(): Promise<APIResponse<{
    total: number
    completed: number
    pending: number
    overdue: number
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : 'Failed to fetch todo statistics')
    }
  }
   ...
   */
  private static calculateXPReward(priority: 'low' | 'medium' | 'high', estimatedMinutes: number): number {
